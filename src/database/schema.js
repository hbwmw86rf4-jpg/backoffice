...
function getDb() {
  ...
  // Run migrations immediately on connection so any code path gets the right schema
  try {
    const syncCols = db.prepare("PRAGMA table_info(pos_sync_log)").all().map(c => c.name);
    if (!syncCols.includes('filename')) {
      db.exec("DROP TABLE IF EXISTS pos_sync_log");
      db.exec(`
      ...
      `);
      db.exec("CREATE INDEX IF NOT EXISTS idx_pos_sync_log_sent ON pos_sync_log(sent_at)");
      db.exec("CREATE INDEX IF NOT EXISTS idx_pos_sync_log_filename ON pos_sync_log(filename)");
      console.log('[DB] Migrated pos_sync_log table');
    }
  } catch (e) {
    console.error('[DB] pos_sync_log migration failed:', e.message);
  }
...
function initializeDatabase() {
  ...
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_pos_sync_log_sent ON pos_sync_log(sent_at);
    CREATE INDEX IF NOT EXISTS idx_pos_sync_log_filename ON pos_sync_log(filename);
  ...
  // Migration: add is_voided to transactions if missing
  const txCols = database.prepare("PRAGMA table_info(transactions)").all().map(c => c.name);
  if (!txCols.includes('is_voided')) {
    database.exec("ALTER TABLE transactions ADD COLUMN is_voided INTEGER DEFAULT 0");
    console.log('[DB] Migrated transactions table: added is_voided');
  }
  ...
  // Migration: upgrade pos_sync_log to detailed tracking
  try {
    const syncCols = database.prepare("PRAGMA table_info(pos_sync_log)").all().map(c => c.name);
    if (!syncCols.includes('filename')) {
      database.exec("DROP TABLE IF EXISTS pos_sync_log");
      database.exec(`
        CREATE TABLE IF NOT EXISTS pos_sync_log (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          filename TEXT NOT NULL,
      ...
      `);
      database.exec("CREATE INDEX IF NOT EXISTS idx_pos_sync_log_sent ON pos_sync_log(sent_at)");
      database.exec("CREATE INDEX IF NOT EXISTS idx_pos_sync_log_filename ON pos_sync_log(filename)");
    }
  } catch (e) {
    console.error('pos_sync_log migration failed:', e.message);
  }
