...
ipcMain.handle('get-dashboard-data', async (event, date) => {
  ...
  const sales = db.prepare(`
    ...
    FROM transactions WHERE business_date = ? AND COALESCE(is_voided, 0) = 0 AND COALESCE(is_training, 0) = 0
...
ipcMain.handle('update-pricebook-item', async (event, id, updates) => {
  ...
  if (updates.price !== undefined && oldPrice !== null && oldPrice !== updates.price) {
    db.prepare('INSERT INTO price_history (pricebook_id, old_price, new_price) VALUES (?, ?, ?)').run(id, oldPrice, updates.price);
  }
...
ipcMain.handle('add-pricebook-item', async (event, item) => {
  ...
  db.prepare('INSERT INTO price_history (pricebook_id, old_price, new_price) VALUES (?, ?, ?)').run(result.lastInsertRowid, 0, item.price || 0);
...
ipcMain.handle('bulk-update-items', async (event, updates) => {
  ...
  // Collect old prices BEFORE any updates
  const oldPrices = {};
  for (const item of updates) {
    if (item.changes.price !== undefined) {
      const row = db.prepare('SELECT price FROM pricebook WHERE id = ?').get(item.id);
      oldPrices[item.id] = row ? row.price : 0;
    }
  }
  ...
  for (const item of updates) {
    for (const [field, value] of Object.entries(item.changes)) {
      if (stmts[field]) {
        ...
        count++;
        if (field === 'price' && oldPrices[item.id] !== value) {
          db.prepare('INSERT INTO price_history (pricebook_id, old_price, new_price) VALUES (?, ?, ?)').run(item.id, oldPrices[item.id], value);
        }
      }
    }
  }
...
ipcMain.handle('get-pos-sync-log', async (event, limit) => {
  return posSender.getPosSyncLog(limit);
});
...
ipcMain.handle('scan-all-outbox', async () => {
  return posWatcher.scanAll();
});
...
// === DAILY RECONCILIATION EXCEL GENERATOR ===
ipcMain.handle('generate-daily-recon', async (event, date) => {
  const { execSync } = require('child_process');
  const targetDate = date || getLocalDate();
  const scriptPath = path.join(__dirname, '..', 'generate_daily_recon.py');
  try {
    const out = execSync(`python "${scriptPath}" ${targetDate}`, { encoding: 'utf-8' });
    const reconPath = path.join(__dirname, '..', 'data', 'recon', `RochesterShell_DailyRecon_${targetDate}.xlsx`);
    return { success: true, path: reconPath, output: out };
  } catch (e) {
