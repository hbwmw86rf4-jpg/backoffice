...
async function checkAckFiles() {
  ...
  if (result.ackFiles.length === 0) {
    content.innerHTML = '<div class="empty-state">No ACK files found in BOOutBox</div>';
  } else {
    content.innerHTML = result.ackFiles.map(f => `
      <div class="import-card">
        <h4>${f.filename}</h4>
        <pre style="max-height: 200px; overflow: auto; background: #f1f5f9; padding: 10px; border-radius: 4px;">${f.content}</pre>
      </div>
    `).join('');
  }
}
